...
static constexpr double PANDALINK0_TO_TASKBOARD_HEIGHT = 0.1035;
...
class Filter
{
  ...
  public:
    Filter(ros::NodeHandle *nh)
    {
        filtering_  = false;
        state_      = 0;
        subscriber_ = nh->subscribe("/startFilterRefined", 1000, &Filter::tf_Callback, this);
    }
    ...
    void startFiltering(tf2_ros::Buffer &tf_buffer)
    {
        ...
        if (state_ == 1)
        {
            ...
            if (tf_buffer.canTransform("panda_link0", "taskboard_center_ref", ros::Time(0), ros::Duration(1), &err_string) &&
                current_col_ < filter_size_)
            {

                auto center                                  = tf_buffer.lookupTransform("panda_link0", "taskboard_center_ref", ros::Time(0));
            ...
            if (current_col_ == filter_size_)
            {
                if (!filtering_)
                {
                    ...
                    stable_center_tf_.transform.translation.z = PANDALINK0_TO_TASKBOARD_HEIGHT;
                    ...
                    stable_center_tf_.child_frame_id  = "taskboard_refined_stable";
...
{
    ros::init(argc, argv, "taskboard_refined_filter");
    ros::NodeHandle nh("taskboard_refined_filter");
